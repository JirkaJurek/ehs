  ...
  <v-data-table :items="items" hide-actions class="elevation-1">
    ...
    <template slot="items" slot-scope="props">
      ...
      <td class="text-xs-center">
        <v-text-field v-model="props.item.number" :min="0" @change="changeCount(props.item)" :max="props.item.inStock" :rules="numberRules(props.item.inStock)" type="number"></v-text-field>
      </td>
    </template>
